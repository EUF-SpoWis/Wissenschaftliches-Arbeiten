...
* **Reduktion:** Die Verringerung von Komplexität erfolgt erst Schritt für Schritt im Prozess der Datenanalyse, nicht vorab bei der Erhebung.

> **Merke:** 
> Eine *Forschungsfrage* bestimmt das wissenschaftliche Erkenntnisziel und ist klar zu unterscheiden von etwa konkreten *Interviewfragen* (Konkretisierung im Erhebungsinstrument) sowie dem *globalen Forschungsthema*.
...
let kommentare = [
  "❌ Diese Frage könnte zu direkt und wertend für den Einstieg sein und das Kind unter Druck setzen, eine bestimmte Antwort zu geben.",
  "❌ Diese Frage ist nicht altersgerecht formuliert und könnte daher Distanz erzeuge und einschüchternd auf das Kind wirken.",
...
let kommentare = [
  "✅Merke: Sprich die Sprache deines Gegenübers, nicht deine eigene Fachsprache. Klare, altersgerechte Fragen verhindern Scheinverhältnisse und soziale Erwünschtheit. „Die Sprache des Befragten sollte als Verständigungsbasis genommen werden.“ (Krieger, 2008, S. 60)",
...
- [( )] Sobald du denkst, dass das Kind keine Worte findet, hilfst du ihm mit möglichen Antworten aus.
<script>
let input = Number("@input");
...
let kommentare = [
  "❌ Diese Reaktion wertet die Aussage des Kindes indirekt ab, indem sie ihm widerspricht. Das Kind könnte daraus ableiten, dass seine ehrliche Aussage nicht willkommen war, und beim nächsten Mal vielleicht eher vorsichtiger antworten.",
  "❌ Eine klare Grenzüberschreitung. Solche wertenden Zuschreibungen können das Vertrauen gefährden und dazu führen, dass das Kind sich in Zukunft gar nicht mehr traut, ehrliche Aussagen zu treffen. ",
  "✅ Merke: Neugier zeigen, statt Urteil bilden. Werte, Motive und Entscheidungen des Kindes sollten nie in Frage gestellt werden; nur so bleibt das Vertrauen (Arbeitsbündnis) erhalten. „Ein Befragter wird immer dann Informationen zurückhalten, beschönigen oder anderweitig verdecken, wenn er sich unsicher fühlt, wenn er der Meinung ist, der Forscher überschreitet Grenzen oder wenn er sich nicht respektiert fühlt.“ (Richartz, 2008, S. 24).", 
  "❌ Diese Aussage mag vielleicht beruhigend gemeint sein, verallgemeinert aber die individuelle Erfahrung des Kindes, wodurch signalisiert werden könnte, dass diese Aussage nichts Besonderes ist. Das Gespräch könnte so an dieser Stelle abgewürgt werden, statt vertieft zu werden."
];
...
let kommentare = [
  "❌ Durch die eigene positive Wertung könnte hier Druck auf das Kind ausgeübt werden, ebenfalls etwas Positives zu berichten. Eine kritische Sichtweise wird dadurch unwahrscheinlicher. Dies nennt man auch das *Phänomen der sozialen Erwünschtheit*",
  "❌ Durch die eigene positive Wertung könnte hier Druck auf das Kind ausgeübt werden, ebenfalls etwas Positives zu berichten. Eine kritische Sichtweise wird dadurch unwahrscheinlicher. Dies nennt man auch das *Phänomen der sozialen Erwünschtheit*",
  "❌ Durch die eigene positive Wertung könnte hier Druck auf das Kind ausgeübt werden, ebenfalls etwas Positives zu berichten. Eine kritische Sichtweise wird dadurch unwahrscheinlicher. Dies nennt man auch das *Phänomen der sozialen Erwünschtheit*",
  "✅ Merke: Nicht bewerten, sondern nachfragen. Konkrete Nachfragen öffnen das Gespräch, ohne zu drängeln. Eine eigene Zustimmung oder Fragen nach 'Ehrlichkeit' können Druck auf das Kind erzeugen, den Ansichten des Interviewenden zuzustimmen. Dies nennt man auch das *Phänomen der sozialen Erwünschtheit*, welches es zu vermeiden gilt.", 
];
...
let kommentare = [
  "❌ Auch wenn es wichtig ist, dem Kind zu folgen und Raum zu geben, sollte irgendwann eine Rückbesinnung auf das eigentliche Thema erfolgen. In diesem Beispiel driftet das Kind nun weiter in eher irrelevante Themen ab. Dadurch könnte es dem Interview an inhaltlicher Struktur mangeln und am Ende nicht die gewünschten Informationen liefern.",
  ...
  "❌ Diese Reaktion mag zwar höflicher als andere Optionen wirken, ist im Kern aber eine verdeckte Form der Leitfadenbürokratie. Der Erzählfluss des Kindes zählt hier fälschlicherweise weniger als das Abhaken der Leitfadenpunkte."
];
